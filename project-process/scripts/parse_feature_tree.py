...
규칙:
  ...
  * 태그는 대괄호 [키: 값] — 유형(검증유형), P(우선순위), 출처(REF/ADD),
    ...
    상태(2026-08-04 신설 — 그 잎을 확인해야 하는 게이팅·세션 상태를 `·` 구분으로 나열.
    선언된 상태마다 최소 한 케이스가 요구된다. 값은 상태 스위처 5종 라벨 그대로:
    미로그인 / 본인인증 미진행 / 성인 인증 / 미성년 / 세션 만료.
    선언 배치 규칙은 rules/depth-and-tn.md §상태 축).
...
def parse(md_text):
    ...
    for raw in md_text.splitlines():
        ...
        node = {
            ...
            "source": tags.get("출처", ""),
            "scope": tags.get("범위", ""),
            # 상태 선언 — 이 잎을 확인해야 하는 상태 목록("·" 구분 원문 그대로).
            # 커버리지 대조(check_tc_coverage.py)가 잎×상태 조합을 요구하는 근거다
            "states": tags.get("상태", ""),
            "pre": [],
            "note": note,
        }
