...
| 상태 축 | 게이팅·세션 상태처럼 여러 잎을 가로지르는 횡단 축. 트리 잎의 [상태:] 선언과 TC 상태 열이 맞물려 잎×상태 조합의 커버리지를 요구하고, 같은 값이 상태별 TC 추출 축이 된다 | `rules/depth-and-tn.md` §상태 축 |
| 기생 상태 | 자기 잎이 없어 트리에 집이 없는 상태. 동작이 갈리는 기능 잎마다 [상태:]로 선언해야 하며, 선언이 그 상태의 유일한 커버리지 요구 지점이다 | `rules/depth-and-tn.md` §상태 축 |
