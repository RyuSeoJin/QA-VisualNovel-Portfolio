...
change-log가 담당합니다. 갱신일: 2026-08-05 (산출물 묶음 해체 — 문서 전부가 `intro/`로 모였고 프로젝트 허브는 폐지했습니다. CI·Pages 통과 확인. **남은 판단 셋은 §사용자 결정 대기에 있습니다**).
...
1. **`gen_project_hub_html.py`를 폐지할까요?** (2026-08-05)
   프로젝트 허브를 없애면서 이 생성기는 **만들 대상을 잃었습니다.** 그런데 「중앙 규칙 구조」
   페이지의 도구 목록은 `scripts/` 폴더를 훑어 만들므로 여전히 목록에 나옵니다 — 쓰이지 않는
   도구가 소개에 실려 있는 상태입니다. 지우면 목록이 정확해지고, 남기면 허브를 되살릴 때
   그대로 씁니다. 되살릴 계획이 없다면 **폐지를 권합니다.**

2. **소개 세 페이지와 자동화 QA 리포트의 수치·표 중복을 어떻게 정리할까요?** (2026-08-05)
   규칙(`rules/site-structure.md` §문서의 세 층)은 「진입 층은 판단과 이유를 말하고 수치·표는
...
3. **그림 안 텍스트 30곳을 언제 고칠까요?** (2026-08-04 보류 → 대기 중)
