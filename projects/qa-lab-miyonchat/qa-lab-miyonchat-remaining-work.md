...
1. **SUT 구현** (`sut/`) — 진행 중. 현재 빌드 `PC웹_Ver1.0_Dev_RC3`
   - **완료**: 전역 셸(상단 바·하단 내비·푸터) · S1 로그인 · 라우팅 가드(공개 S2·S7 / 보호 S3·S4·S5·S6·S8) · P4 간편 프로필 · T1 디버그 콘솔 · 가상 시계 · `__VN__` API · S2 홈(필터 칩·추천/랭킹/신작/카테고리·캐릭터 카드·카드 상세·언세이프 게이팅)
   - **상단 바 잔여 2노드** — 키워드 검색(제목 부분일치·0건 안내, 결과는 S2에 표시)과 알림 목록 표시. 둘 다 탐색/발견 영역이고 S2·상단 바에 붙으므로 S3보다 먼저 처리하는 편이 재작업이 적습니다(순서는 사용자 결정)
