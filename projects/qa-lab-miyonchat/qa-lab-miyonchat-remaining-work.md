...
change-log가 담당합니다. 갱신일: 2026-08-02 (SUT 첫 슬라이스·디버그 콘솔 완료 시점).

## 다음 작업 큐

1. **SUT 구현** (`sut/`) — 진행 중. 현재 빌드 `PC웹_Ver1.0_Dev_RC2`
   - **완료**: 전역 셸(상단 바·하단 내비·푸터) · S1 로그인 · 라우팅 가드(공개 S2·S7 / 보호 S3·S4·S5·S6·S8) · P4 간편 프로필 · T1 디버그 콘솔 · 가상 시계 · `__VN__` API
   - **다음 단위 — S2 홈**: 필터 칩 4종(추천·랭킹·신작·카테고리) → 추천 섹션(캐러셀 최대 7·최근 대화 조건부 노출·떠오르는 신작·지금 뜨거운) → 랭킹(기간 일/주/월 · 기준 4종 · 리뷰 50 최소 표본) → 카테고리 공통 템플릿(인기 신작 → 취향 태그 필터 → 전체 목록 대화순/최신순) → 캐릭터 카드와 게이팅(미로그인·본인인증 미진행·미성년은 언세이프 블러)
   - 그 뒤: S3 페르소나 → S4 대화 → P1·P2·P3 패널 → S5 채팅 → S6 MY → 스텁(S7·S8)
...
- **미push 커밋 push 시점** — 2026-08-02 `3567cdd`까지 push 완료. 이후 누적분(골격 v1.2·SUT 첫 슬라이스·이슈 체계·디버그 콘솔)은 대기 중이며 저작권 게이트에 걸릴 항목 없음
