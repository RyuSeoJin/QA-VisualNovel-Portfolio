...
1. **SUT 구현** (`sut/`) — 진행 중. 현재 빌드 `PC웹_Ver1.0_Dev_RC4`
   - **완료**: 전역 셸(상단 바·하단 내비·푸터) · S1 로그인 · 라우팅 가드(공개 S2·S7 / 보호 S3·S4·S5·S6·S8) · P4 간편 프로필 · T1 디버그 콘솔 · 가상 시계 · `__VN__` API · S2 홈(필터 칩·추천/랭킹/신작/카테고리·캐릭터 카드·카드 상세·언세이프 게이팅) · 상단 바 검색·알림 — **탐색/발견 영역 전부**
   - **다음 단위 — S3 페르소나**: 이름·호칭·성별·자유 설명 폼 → 글자수 카운터(12/12/1,000) → 필수값 없으면 저장 비활성 → 저장 후 카드 상세에서 넘어온 시작점(`pendingStart`)으로 대화 개시
   - 그 뒤: S4 대화 → P1·P2·P3 패널 → S5 채팅 → S6 MY(세이프티 필터 토글 포함) → 스텁(S7·S8)
