...
change-log가 담당합니다. 갱신일: 2026-08-05 (`gen_project_hub_html.py` 폐지 + 허브 표기 정리 — 소개의 도구 목록에서 빠졌고, 규칙 문서에 남아 있던 허브 설명과 **404였던 링크 둘**을 「프로젝트 개요」로 맞췄습니다. 산출물 층의 자리 표기도 실제(`intro/`)에 맞췄습니다. **남은 판단 둘은 §사용자 결정 대기에 있습니다**).
...
| 다음 착수 지점 | **작업 큐가 비었습니다.** 남은 판단 둘(§사용자 결정 대기)을 먼저 정하고, 그다음은 §백로그에서 고릅니다 |
...
1. **소개 세 페이지와 자동화 QA 리포트의 수치·표 중복을 어떻게 정리할까요?** (2026-08-05)
   규칙(`rules/site-structure.md` §문서의 층)은 「진입 층은 판단과 이유를 말하고 수치·표는
   산출물로 보낸다」인데 지금은 어긋나 있습니다. 겹치는 자리는 넷입니다 — 커버리지 3축(TC 설계
   §「다 봤다」 2,203자) · 결함 주입 매트릭스(자동화 §고장을 심어 2,256자) · 영역별 규모 ·
   검증유형 건수. 갈래는 셋입니다.
   - **(가) 규칙대로 되돌리기** — 소개는 이유만 남기고 수치·표는 리포트로. 소개가 짧아지고
     숫자가 갈라질 자리가 사라집니다(권장)
   - (나) 리포트를 줄이기 — 리포트는 **CI가 최신 여부를 검사하는 유일한 산출물**이라
     권하지 않습니다
   - (다) 그대로 두기 — 의도적 중복으로 보되, 소개의 수치도 정본에서 읽어 오게 바꿔야
     갈라지지 않습니다
   **주의**: 소개 층 문구는 동결이므로 (가)·(다) 어느 쪽이든 사용자 승인 후에 손댑니다.

2. **그림 안 텍스트 30곳을 언제 고칠까요?** (2026-08-04 보류 → 대기 중)
