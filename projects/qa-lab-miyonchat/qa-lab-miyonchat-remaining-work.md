...
change-log가 담당합니다. 갱신일: 2026-08-03 (골격 v1.11 · SUT RC21 · TC 「웹 진입」 11건 완료 · 남은 9영역).
...
| 골격 | **v1.11** — 미확인 목록이 비었고 보호 화면 목록이 명세와 맞습니다 |
| git | **push 완료 · 미push 없음.** 좌표는 `git log --oneline -1`·`git status -sb`로 확인합니다 |
| 다음 착수 지점 | **TC 본문 — 나머지 9영역.** 첫 영역 「웹 진입」은 11건 23행으로 완료(형식·밀도 확인받음). `d1_order`의 2번 「전역 셸」부터 잇습니다 |
...
2. **TC 본문 작성** (playbook STEP 6) — **다음 단위. 설계 규칙은 전부 확정, 쓰기만 남음**
   ...
   - **작업 순서**: ~~① 「웹 진입」 영역 작성 → 형식·밀도 확인~~(완료 — 11건 23행) →
     ② 나머지 9영역 → ③ xlsx 생성 → ④ Excel 오픈 검증
   - 첫 영역에서 확정된 것: **TN은 연쇄에만** 쓰고 나열은 케이스로 나눕니다 · 기대결과는
     **스텝별 배열**로 적습니다 · 영역코드는 `area_codes`가 정본입니다
