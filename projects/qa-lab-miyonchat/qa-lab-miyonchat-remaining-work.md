...
change-log가 담당합니다. 갱신일: 2026-08-04 (결함 주입 매트릭스 통과 — 대각선만 FAIL · TC 152건 157건 통과 · RC24 · 다음은 리포트).
...
| TC·자동화 | TC **152건** · 자동화 **157건 통과** · 커버리지 3축 통과 · **결함 주입 매트릭스 통과**(담당 14건 전부 잡음·담당 밖 0) |
...
4. **리포트** — **다음 착수 지점**
   ...
   - 「SUT 한계와 검증 범위」 재료는 셋입니다 — 청사진 §4 검증 가능성 맵 · §4-1 전개 축 제외
     판정 · `test-case/…-coverage-waiver.json`의 제외 7건(각 항목에 `kind`·사유가 있고
     `requires`가 있는 것은 근거까지 기계가 확인합니다)
...
칸에 놓이며, 생성 시 **실제로 쓰인 깊이까지만** 뎁스 열이 만들어집니다. 13번째가 `covers`
