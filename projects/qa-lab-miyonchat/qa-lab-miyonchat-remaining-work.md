...
change-log가 담당합니다. 갱신일: 2026-08-03 (결정 대기 전부 해소 · 골격 v1.10 · SUT RC21 슬라이스 등록).
...
| 골격 | **v1.10** — 미확인 목록이 비었습니다 |
| git | **push 완료 · 미push 없음.** 좌표는 `git log --oneline -1`·`git status -sb`로 확인합니다 |
| 다음 착수 지점 | **둘 중 하나입니다.** ① **SUT RC21** — 2026-08-03 확정된 §8-8 두 값을 코드에 반영(1번 항목 「남은 슬라이스」) ② **TC 본문 작성 — 첫 영역 「웹 진입」**(`tcs` 배열만 비어 있음). ②는 ①에 걸리지 않지만, 「탐색」 영역 TC 전에는 ①이 끝나 있어야 합니다 |
...
1. **SUT 구현** (`sut/`) — **완료(RC20)**
   ...
   - 지켜야 할 조건: ① 코드가 명세와 어긋나면 **명세를 먼저 고치고** 스펙 변경으로 기록
     ...
     전에 확정
   - **남은 슬라이스 — RC21(§8-8 명세 반영).** 2026-08-03에 확정된 캐릭터 페이지 두 값을 코드가
     아직 따라가지 못합니다. 「웹 진입」 영역 TC는 막지 않지만, 「탐색」 영역 TC를 쓰기 전에는
     끝나 있어야 기대값과 화면이 맞습니다
     - 디버그 콘솔 표에 **최종 업데이트일·버전 편집 칸** 신설 — 값(`updatedDay`·`version`)은
       이미 있는데 표에 열이 없어 고칠 수단이 없습니다. 버전 입력은 **숫자만** 받고 표시할 때
       `v`를 붙입니다(`1.1` → `v1.1`, 접두를 붙여 입력해도 겹치지 않게)
     - **작품 추천 선정식 교체** — 현재는 같은 카테고리에서 체인 순 4개입니다. 확정 규칙은
       카테고리를 하나라도 공유 + **좋아요 10 이상 · 최대 5개**이고 순서는 좋아요를 선택
       기준으로 §8-4 동률 체인입니다
     - **후보 0건일 때 「관련 추천 작품이 없습니다」 표시** — 현재는 섹션을 통째로 숨깁니다.
       기본 데이터에서 자주 나오는 화면이라 testid를 새로 붙입니다
     - **좋아요 임계·노출 상한을 디버그 콘솔에서 조절** — 기본값 10·5가 확정값이고 콘솔 조절은
       경계 확인 수단입니다(system-spec §8-8에 명시)
     - **용어 통일** — 콘솔 표 헤더 「생성일」과 변경 요약 라벨을 **「출시일」**로. 내부 필드명
       `createdDay`는 그대로 둡니다
     - 마무리: `SUT_BUILD`를 RC21로 · `?v=` 올리기 · 신설 testid를 청사진 §3-1에 등재
...
2. **TC 본문 작성** (playbook STEP 6) — **다음 단위. 설계 규칙은 전부 확정, 쓰기만 남음**
   ...
   - **작업 순서**: ① 「웹 진입」 영역 작성 → **여기서 멈추고 형식·밀도 확인받기** →
     ② 나머지 9영역 → ③ xlsx 생성 → ④ Excel 오픈 검증
...
3. **자동화** — conftest(reset)·thresholds·영역별 테스트 파일 → 결정적·금칙 격리부터 →
   ...
   - **커버리지 대조 스크립트**를 여기서 만듭니다 — 구현 85잎 ↔ TC를 맞춰 **미매핑 잎을
     목록으로** 뱉습니다. 뎁스가 도달 경로로 바뀌어 트리 영역이 여러 1-Depth로 흩어지므로,
     "빠짐없이 썼나"를 눈으로 확인할 수 없게 되었습니다. 결함 주입 매트릭스의 열 매핑도
     같은 자리에서 처리합니다
