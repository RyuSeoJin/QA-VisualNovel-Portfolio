...
change-log가 담당합니다. 갱신일: 2026-08-05 (`gen_project_hub_html.py` 폐지 + 허브 표기 정리 — 소개의 도구 목록에서 빠졌고, 규칙 문서에 남아 있던 허브 설명과 **404였던 링크 둘**을 「프로젝트 개요」로 맞췄습니다. **남은 판단 셋은 §사용자 결정 대기에 있습니다**).
...
| 다음 착수 지점 | **작업 큐가 비었습니다.** 남은 판단 셋(§사용자 결정 대기)을 먼저 정하고, 그다음은 §백로그에서 고릅니다 |
...
3. **산출물 층의 「자리」를 어디로 적을까요?** (2026-08-05)
   `rules/site-structure.md` §문서의 층에서 산출물 행이 아직 **「프로젝트 폴더 안」**이라고
   말합니다. 그런데 2026-08-05 묶음 해체로 리포트·추적 매트릭스·기능 골격·용어집 HTML은
   **전부 `intro/`로 옮겨 갔고**, 프로젝트 폴더에 남은 산출물은 TC 시트(xlsx)뿐입니다.
   허브 정리와 같은 표에 있지만 **허브와 무관한 별개의 낡음**이라 이번에 손대지 않았습니다.
   - **(가) 자리를 둘로 갈라 적기** — 「읽는 HTML은 `intro/`, 원본 파일은 프로젝트 폴더」.
     실제와 맞고 Pages에서 보이는 것과도 같습니다(권장)
   - (나) 그대로 두기 — xlsx 기준으로 읽으면 틀린 말은 아니지만, 이 표를 보고 새 HTML
     산출물의 자리를 정하면 옛 자리에 놓게 됩니다
