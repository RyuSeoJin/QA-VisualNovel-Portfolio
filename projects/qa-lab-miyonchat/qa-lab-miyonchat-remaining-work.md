...
change-log가 담당합니다. 갱신일: 2026-08-01 (SUT 청사진 확정 직후 기준).

## 다음 작업 큐

1. **design 명세 작성** — 입력: 트리 미확인 목록 13건 + 채택표 §12 수치 후보
...
3. **SUT 구현** (`sut/`) — 청사진 준수, data-testid 전체 목록을 청사진에 확정 등재, 스모크의 STUB를 실제 SUT로 교체(test_smoke.py [주의] 참조)
4. **자동화** — conftest(reset)·thresholds·영역별 테스트 파일 → 결정적·금칙 격리부터 → 결함 주입 매트릭스(대각선만 FAIL) → 리포트(+"SUT 한계와 검증 범위")
5. **CI·묶기** — 워크플로(경로 필터, deploy-pages 금지), 추적 매트릭스, 프로젝트 허브·용어집 생성, README 실행 GIF, 저작권 게이트 → push

## 사용자 결정 대기

- **Issue 시트 포맷** — 사용자가 직접 제공 예정. tc-sheet-master.xlsx에 추가 후 명세서 시트 갱신 (2번 TC 산출 전까지 필요)
- **자동화 리포트 템플릿(Case C)** — 기본 리포트로 시작 권고 유지, 여유 시 재논의 (4번 리포트 단계 전까지)
