...
- `projects/{프로젝트}/` — 프로젝트별 산출물. 허브(`{프로젝트}-index.html`)·용어집(`{프로젝트}-dictionary.html`)·이력(`{프로젝트}-change-log.md`) + `analysis/`(조사 전량)·`reference/`(채택분)·`spec/`(확정 결정 — 정본이 사는 곳. 평면에는 골격 트리 정본만 두고, 구현·검증 사양은 `design/`, 근거는 `rationale/`, 지나간 것은 `archive/`로 갈라 둡니다)·`test-case/`. 테스트 대상을 직접 만드는 프로젝트는 `sut/`(테스트 대상)·`automation/`(자동화 스크립트·리포트)을 더합니다.
...
| `spec/design/` | **참조 자유.** 구현·검증 사양(SUT 청사진, 규칙 수치, 세이브 구조, mock 사양, 결함 주입, 루브릭)이 사는 곳. **TC 기대값은 평면의 트리와 여기에서만 가져옵니다** |
| `spec/rationale/` | **참조 자유.** 레퍼런스에 없어 직접 세운 노드와 직접 정한 수치의 근거가 모이는 곳이므로 작업 중 수시로 읽습니다. 판단 기록이지 확정안이 아니므로 TC 기대값으로 쓰지 않습니다 |
