...
- `projects/{프로젝트}/` — 프로젝트별 산출물. 허브(`{프로젝트}-index.html`)·용어집(`{프로젝트}-dictionary.html`)·이력(`{프로젝트}-change-log.md`) + `analysis/`(조사 전량)·`reference/`(채택분)·`spec/`(확정 결정 — 정본이 사는 곳. 평면에는 확정안만 두고 근거는 `rationale/`, 지나간 것은 `archive/`로 갈라 둡니다)·`test-case/`. 테스트 대상을 직접 만드는 프로젝트는 `sut/`(테스트 대상)·`automation/`(자동화 스크립트·리포트)을 더합니다.
...
| `spec/` 평면 | 확정안이 사는 곳이므로 항상 참조합니다. TC의 기대값은 여기서만 가져옵니다 |
| `spec/rationale/` | **참조 자유.** 레퍼런스에 없어 직접 세운 노드와 직접 정한 수치의 근거가 모이는 곳이므로 작업 중 수시로 읽습니다 |
